# Licence

## Course text and site content

All prose, tables, checklists and page markup in this repository — everything except the files noted
below — is licensed under the
**[Creative Commons Attribution 4.0 International licence (CC BY 4.0)](https://creativecommons.org/licenses/by/4.0/)**.

You may share and adapt it, including commercially, provided you give appropriate credit, link to
the licence, and indicate whether changes were made.

Attribution suggestion:

> "Bonsai Upskilling" by integration-innovation, CC BY 4.0 —
> https://integration-innovation.github.io/bonsaiupskilling

## Example scripts

Everything under `exercises/` is licensed **GPL-3.0-or-later**, matching
[Bonsai](https://bonsaibim.org/) and
[Bonsai Sketch Mode](https://github.com/integrations-space/BonsaiSketch), which those scripts are
written to run against.

## The Farnsworth House

The course project is the **Edith Farnsworth House** (Ludwig Mies van der Rohe, Plano, Illinois,
1945–51). The position, in short: modelling and publishing it is fine, and photographs are not
included.

**The building.** The US
[Architectural Works Copyright Protection Act](https://www.copyright.gov/circs/circ41.pdf) took
effect on **1 December 1990** and does not extend to works constructed before that date. The house
was completed in 1951, so as an architectural work it carries no US copyright that a model of it
could infringe. The reference model and the massing script are therefore released under the same
GPL-3.0-or-later terms as the rest of `exercises/`.

**The HABS record.** The house is documented as **HABS IL-1105** — 8 measured drawings, 32
photographs and 54 data pages — held by the Library of Congress. Historic American Buildings Survey
documentation is prepared by the US National Park Service, and everything transmitted to HABS is
placed in the public domain as a condition of the transfer; the Library of Congress rights advisory
reads "no known restrictions on publication". **This covers the HABS photographs as well as the
HABS drawings.** The course links to the collection and asks you to fetch it; it is not mirrored
here.

**Mies's own drawings — a separate and live copyright.** Architectural drawings are protected as
pictorial and graphic works under 17 U.S.C. §102(a)(5), independently of the building they depict,
and the AWCPA's pre-1990 cut-off does not release them. The Farnsworth House drawings are held in
the Mies van der Rohe Archive at the Museum of Modern Art, and their copyright is administered by
Artists Rights Society (ARS), New York / VG Bild-Kunst, Bonn; reproduction is handled by Art
Resource or Scala. **No Mies drawing is reproduced, traced or redrawn in this repository**, and
neither the reference model nor the massing script derives from one. They are built from published
dimensions, which are facts and not subject to copyright.

The same restriction applies to the redrawn plans and sections that circulate on architecture
websites: those are derivative works of the original drawings, and being freely visible is not the
same as being freely usable.

The `sources.md` page in this repository sets out the equivalent position for other architects —
which drawings are genuinely royalty-free, which are administered estates, and how the three
questions (building, drawing, photograph) come apart.

**Other photographs.** Photographs of the house that are *not* part of the HABS record — press,
magazine, and modern architectural photography — are separately protected, whoever owns the
building. No such photograph is reproduced here, and you should not add one without checking its
licence.

**Furniture.** The Mies-designed furniture associated with the house is protected separately as
design, and is deliberately not modelled.

**The house today.** It is owned and operated by the
[National Trust for Historic Preservation](https://savingplaces.org/). This project is not
affiliated with, endorsed by, or connected to the Trust or the Edith Farnsworth House.

Dimensions used in the reference model come from published secondary sources, not from the HABS
drawings, and each carries its source and a confidence grade in the model itself. They are a
teaching approximation and must not be relied on for any purpose connected with the actual building.

## Third-party material

The **SIA Scope of Service Matrix** (Architect's Appointment Committee, updated 14 August 2018) and
the **SIA Value Articulation Framework / SIA BluePrint V.1** (2024) are published by the
**Singapore Institute of Architects** and remain their copyright. Neither document is reproduced
here. The stage descriptions on this site are our own paraphrase, written for teaching, and may be
incomplete or out of date.

**IFC+SG**, the **General Modelling Practices** and the **CORENET X Code of Practice** are published
by the Government of Singapore via [CORENET X](https://info.corenet.gov.sg/). Nothing from those
pages is reproduced here; the summaries are our own, written for teaching, and the source is
authoritative.

This project is not published by, endorsed by, or affiliated with the Singapore Institute of
Architects, the Building and Construction Authority, the Government of Singapore, the National Trust
for Historic Preservation, the Bonsai project, or the Blender Foundation.

Nothing here is legal, contractual or regulatory advice. An architect's actual scope of service is
whatever the consultancy agreement states, and authority requirements change. Work from the source
documents.
